document.addEventListener("DOMContentLoaded", () => {
    ...
    const generateId = () => "card-" + new Date().getTime() + Math.floor(Math.random() * 1000);
    ...
    /**
     * Tworzy element DOM dla nowej karty
     * @param {object} cardData - Obiekt z danymi karty (id, title, color)
     * @returns {HTMLElement} Element div.kanban-card
     */
    const createCardElement = (cardData) => {
        ...
        card.append(title, buttonsContainer);
    ...
    const saveBoard = () => {
        ...
        for (const columnId in columnLists) {
            const cards = columnLists[columnId].querySelectorAll(".kanban-card");
            cards.forEach(card => {
                state[columnId].push({
                    id: card.dataset.id,
    ...
    /**
     * Handler do dodawania nowej karty
     * @param {Event} e - Zdarzenie kliknięcia
     */
    const handleAddCard = (e) => {
        const columnList = e.target.closest(".kanban-column").querySelector(".card-list");
    ...
    /**
     * Handler do kolorowania wszystkich kart w kolumnie
     * @param {Event} e - Zdarzenie kliknięcia
     */
    ...
    /**
     * Handler do sortowania kart w kolumnie (alfabetycznie)
     * @param {Event} e - Zdarzenie kliknięcia
     */
    ...
    /**
     * Główny handler do obsługi akcji na kartach (Usuń, Przesuń, Koloruj)
     * Używa delegowania zdarzeń.
     * @param {Event} e - Zdarzenie kliknięcia
     */
    ...
    /**
     * Handler do zapisu zmian po edycji tytułu karty
     * @param {Event} e - Zdarzenie 'blur'
     */
    ...
    columns.forEach(column => {
        column.querySelector(".add-card-btn").addEventListener("click", handleAddCard);
        column.querySelector(".color-column-btn").addEventListener("click", handleColorColumn);
        column.querySelector(".sort-column-btn").addEventListener("click", handleSortColumn);
    });
